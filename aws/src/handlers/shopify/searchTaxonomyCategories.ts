...
import { requireQueryParam, jsonError, jsonOk, readIntegerQueryParam } from '../../shared/http.js';
...
export async function handler(event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> {
  try {
    ...
    return jsonError(getStatusCode(error), toApiErrorBody('shopify', error, 'SHOPIFY_SEARCH_TAXONOMY_FAILED'));
  }
}