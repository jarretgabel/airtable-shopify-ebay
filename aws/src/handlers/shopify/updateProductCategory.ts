...
export async function handler(event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> {
  try {
    ...
    if (!Number.isInteger(productId) || productId <= 0) {
      return jsonError(400, toApiErrorBody('shopify', new Error('productId must be a positive integer'), 'INVALID_PRODUCT_ID'));
    }

    const body = requireJsonBody<UpdateProductCategoryBody>(event, 'shopify', 'INVALID_SHOPIFY_REQUEST_BODY');
    if (!body.categoryId?.trim()) {
      return jsonError(400, toApiErrorBody('shopify', new Error('categoryId is required'), 'MISSING_CATEGORY_ID'));
    }
    ...
    return jsonError(getStatusCode(error), toApiErrorBody('shopify', error, 'SHOPIFY_UPDATE_PRODUCT_CATEGORY_FAILED'));
  }
}