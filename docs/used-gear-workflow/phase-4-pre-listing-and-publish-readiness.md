...
### Goals
- Create an explicit listing-phase operational review step inside Combined Listings.
- Prefill the listing editors as much as possible.
- Make reviewer/pricing confirmation explicit before publish.
- Keep listing review on the same selected-record page that later owns publish and lifecycle follow-through.
...
9. Keep listing review inside the shipped `Intake > Processing > Listing` navigation order instead of adding a separate pre-listing destination.
...
- Listing-phase shell guidance that matches the operator-facing intake and processing pages.
...
- Listing teams have enough dedicated app pages to review, queue, and hand off items without relying on Airtable views directly or a separate pre-listing route.
...
- The Listings selected-record experience now owns reviewer confirmation, blocker messaging, `Approve For Publish` actions, and the operator-facing review guidance that used to imply a separate pre-listing surface.
...
### Implemented Listing-Phase Surface Ownership
- Combined Listings is now the first listing-phase directory after the downstream Testing and Photography holding stage. `Awaiting Pre-Listing Review` is the entry bucket, and `Approved for Publish` plus later lifecycle work remain on the same Listings surface.
- The combined listing queue and selected-record page now reuse the same workflow page-header, guidance-card, and operator-guide pattern already used on Manual Intake and the dedicated Testing and Photography directory pages.
- The app-shell order remains `Intake > Processing > Listing`, so listing review begins only after the dedicated processing surfaces instead of behind a separate pre-listing route.
- Manual-entry parity, grouped intake handling, pricing/allocation behavior, and customer-versus-internal note separation remain preserved because listing review still writes back to the same authoritative Airtable row rather than copying data into a parallel review surface.
