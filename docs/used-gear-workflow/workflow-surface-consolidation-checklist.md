# Workflow Surface Consolidation Checklist
...
- [x] Keep the testing overview route in `src/app/AppTabContent.tsx`.
- [x] Keep the testing queue tab wiring in `src/components/tabs/UsedGearWorkflowQueueTab.tsx` or replace it with a stronger testing directory surface.
- [x] Reuse the intake page template structure for the testing overview page where it fits the processing workflow.
- [x] Ensure testing overview supports filtering, sorting, search, grouped context, and URL-backed state.
- [x] Update any related queue logic in `src/components/tabs/airtable/UsedGearWorkflowProgressSection.tsx`.
- [x] Update tests covering testing overview behavior.

## 5. Preserve And Strengthen Photography Overview Page
- [x] Keep the photography overview route in `src/app/AppTabContent.tsx`.
- [x] Keep the photography queue tab wiring in `src/components/tabs/UsedGearWorkflowQueueTab.tsx` or replace it with a stronger photography directory surface.
- [x] Reuse the intake page template structure for the photography overview page where it fits the processing workflow.
- [x] Ensure photography overview supports filtering, sorting, search, grouped context, and URL-backed state.
- [x] Update any related queue logic in `src/components/tabs/airtable/UsedGearWorkflowProgressSection.tsx`.
- [x] Update tests covering photography overview behavior.

## 6. Enforce Downstream Holding Before Listings
- [x] Define the downstream post-intake parking lot behavior in code and docs so accepted rows clearly move into a shared testing-and-photography holding stage.
- [x] Decide whether that downstream parking lot is represented by existing workflow states and queue pages or by a newly named queue surface.
- [x] Confirm accepted parking-lot rows route into downstream testing-and-photography work rather than Listings.
- [x] Update routing or target generation in `src/services/usedGearQueue.ts` as needed.
- [x] Update queue summaries, notifications, and deep-link targets that currently imply pre-listing belongs outside Listings.
- [x] Update tests in `tests/unit/src/services/usedGearQueue.test.ts` for the new target behavior.
...
- [x] Reuse the intake page template structure for listing-phase directory and selected-record surfaces where it fits the listing workflow.
...
- [x] Extend `src/components/approval/ListingApprovalSelectedRecordPanel.tsx` as the selected-record home for pre-listing review modules.
...
- [x] Preserve per-user in-app notification configurability while repointing workflow notifications.
...
- [x] Update `docs/used-gear-workflow/phase-3-processing-and-handoffs.md` for directory-page ownership and form-only completion.
- [x] Update `docs/used-gear-workflow/phase-4-pre-listing-and-publish-readiness.md` so listing review begins at `Awaiting Pre-Listing Review` inside Listings.
- [x] Update workflow docs to explicitly describe the downstream post-intake parking lot between intake approval and listing-phase entry.
- [x] Update workflow docs to explicitly describe the new navigation order `Intake > Processing > Listing`.
- [x] Update workflow docs to explicitly describe reuse of the intake page template across processing and listing surfaces where applicable.
- [x] Confirm workflow docs still preserve original requirements for manual-entry parity, grouped intake handling, pricing/allocation behavior, and customer-versus-internal note separation.
- [x] Update `docs/used-gear-workflow/ui-surface-map.md`.
- [x] Update `docs/used-gear-workflow/release-smoke-test-checklist.md`.
- [x] Update `src/components/tabs/workflowGuideContent.ts` and any workflow guide tests.
...
- [x] Slice 1: Testing form signoff writes plus removal of duplicate testing and photography completion actions.
- [x] Slice 2: Strengthen Testing and Photography overview pages where needed.
- [x] Slice 3: Add listing-phase queue buckets and migrate pre-listing review modules into listing pages.
- [x] Slice 4: Repoint notifications and dashboard shortcuts from `pre-listing-queue` to Listings.
- [x] Slice 5: Reorganize navigation into `Intake > Processing > Listing` order.
