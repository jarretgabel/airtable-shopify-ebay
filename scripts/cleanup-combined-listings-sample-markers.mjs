...
const APPLY_CONFIRM_TOKEN = 'APPLY_COMBINED_LISTINGS_SAMPLE_MARKER_CLEANUP';
const RUNS_DIR = path.join(process.cwd(), 'tmp', 'combined-listings-sample-marker-cleanup');
const BATCH_SIZE = 10;
...
function printHelp() {
  console.log('Combined listings sample-marker cleanup helper');
  console.log('');
  console.log('Commands:');
  console.log('  plan [--output-dir path]');
  console.log('  apply --plan path/to/cleanup-plan.json --confirm APPLY_COMBINED_LISTINGS_SAMPLE_MARKER_CLEANUP [--output-dir path]');
}
...
function buildCleanupPlan(records) {
  ...
  records.forEach((record) => {
    ...
    Object.entries(fields).forEach(([fieldName, fieldValue]) => {
      ...
      const sanitizedValue = sanitizeMarkerText(fieldValue);
  ...
  planEntries.forEach((entry) => {
    ...
    entry.changedFieldNames.forEach((fieldName) => {
      ...
      if (currentValue !== plannedBeforeValue) {
        drifted.push({ id: entry.id, fieldName, plannedBeforeValue, currentValue });
      }
    });
  });
...
async function runPlan(outputDir) {
  ...
  const summary = {
    ...
    plannedFieldCount: entries.reduce((count, entry) => count + entry.changedFieldNames.length, 0),
  };

  writeJson(path.join(runDir, 'summary.json'), summary);
  writeJson(path.join(runDir, 'cleanup-plan.json'), { ...summary, entries });

  console.log(`Planned cleanup for ${entries.length} combined-listings row(s).`);
...
async function runApply(planPath, confirmToken, outputDir) {
  ...
  writeJson(path.join(runDir, 'summary.json'), {
    ...
    remainingTaggedRecordCount: remainingEntries.length,
  });

  console.log(`Updated ${updatedRecords.length} combined-listings row(s).`);
  console.log(`Remaining tagged combined-listings row(s): ${remainingEntries.length}`);
  console.log(`Artifacts saved in ${runDir}`);
}
