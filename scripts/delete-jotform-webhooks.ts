...
import { deleteFormWebhook, listFormWebhooks } from '../aws/src/providers/jotform/client.js';
...
async function main(): Promise<void> {
  const formId = process.env.JOTFORM_FORM_ID.trim();
  const targetWebhookUrl = buildWebhookUrl();
  const webhooks = await listFormWebhooks(formId);
  const matches = webhooks.filter((webhook) => webhook.webhookUrl === targetWebhookUrl);

  for (const webhook of matches) {
    await deleteFormWebhook(formId, webhook.id);
  }
