    ...
    <DashboardSectionPanel id="inventory" title="Inventory" className="gap-[1.1rem]">
      ...
      {atLoading ? (
        ...
        <>
          ...
          <p className="m-0 text-[0.84rem] leading-[1.55] text-[var(--muted)]">
            Start with the catalog footprint and the most common product clusters. Expand the tables only when you need the fuller distributor or component breakdown.
          </p>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <section className="rounded-[14px] border border-[var(--line)] bg-[var(--bg)] p-4"><h3 className="mb-1 text-[0.9rem] font-bold text-[var(--ink)]">By Component Type</h3><p className="m-0 mb-3 text-[0.78rem] text-[var(--muted)]">Top 6 product categories by count.</p><ul className="m-0 flex list-none flex-col gap-3 p-0">{topComponentTypes.map(([label, count], index) => <li key={`${label}-${index}`} className="grid grid-cols-[minmax(90px,140px)_1fr_32px] items-center gap-3"><span className="truncate whitespace-nowrap text-[0.82rem] text-[var(--ink)]">{label}</span><div className="h-2.5 overflow-hidden rounded-full bg-[var(--line)]"><div className="h-full min-w-[6px] rounded-full bg-gradient-to-r from-teal-500 to-teal-300" style={{ width: `${toPercent(count, maxComponentTypeCount)}%` }} /></div><span className="text-right text-[0.8rem] font-bold text-[var(--ink)] [font-variant-numeric:tabular-nums]">{count}</span></li>)}</ul></section>
            <section className="rounded-[14px] border border-[var(--line)] bg-[var(--bg)] p-4"><h3 className="mb-1 text-[0.9rem] font-bold text-[var(--ink)]">Top Brands</h3><p className="m-0 mb-3 text-[0.78rem] text-[var(--muted)]">Top 6 brands represented in the catalog.</p><ul className="m-0 flex list-none flex-col gap-3 p-0">{topBrands.map(([label, count], index) => <li key={`${label}-${index}`} className="grid grid-cols-[minmax(90px,140px)_1fr_32px] items-center gap-3"><span className="truncate whitespace-nowrap text-[0.82rem] text-[var(--ink)]">{label}</span><div className="h-2.5 overflow-hidden rounded-full bg-[var(--line)]"><div className="h-full min-w-[6px] rounded-full bg-gradient-to-r from-blue-500 to-blue-300" style={{ width: `${toPercent(count, maxAirtableBrandCount)}%` }} /></div><span className="text-right text-[0.8rem] font-bold text-[var(--ink)] [font-variant-numeric:tabular-nums]">{count}</span></li>)}</ul></section>
          </div>

          <details className="mt-1 rounded-[14px] border border-[var(--line)] bg-[color:color-mix(in_srgb,var(--panel)_88%,transparent)] px-4 py-3">
            <summary className="cursor-pointer list-none text-[0.8rem] font-semibold text-[var(--ink)]">Show inventory tables</summary>
            <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
              <section className="rounded-[14px] border border-[var(--line)] bg-[var(--bg)] p-4">
                <h3 className="mb-3 text-[0.9rem] font-bold text-[var(--ink)]">By Distributor</h3>
