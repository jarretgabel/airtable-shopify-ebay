      ...
      {atLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <section className="rounded-[14px] border border-[var(--line)] bg-[var(--bg)] p-4"><h3 className="mb-3 text-[0.9rem] font-bold text-[var(--ink)]">By Component Type</h3><ul className="m-0 flex list-none flex-col gap-3 p-0">{componentTypeSummary.map(([label, count]) => <li key={label} className="grid grid-cols-[minmax(90px,140px)_1fr_32px] items-center gap-3"><span className="truncate whitespace-nowrap text-[0.82rem] text-[var(--ink)]">{label}</span><div className="h-2.5 overflow-hidden rounded-full bg-[var(--line)]"><div className="h-full min-w-[6px] rounded-full bg-gradient-to-r from-teal-500 to-teal-300" style={{ width: `${Math.round((count / maxComponentTypeCount) * 100)}%` }} /></div><span className="text-right text-[0.8rem] font-bold text-[var(--ink)] [font-variant-numeric:tabular-nums]">{count}</span></li>)}</ul></section>
            <section className="rounded-[14px] border border-[var(--line)] bg-[var(--bg)] p-4"><h3 className="mb-3 text-[0.9rem] font-bold text-[var(--ink)]">Top Brands</h3><ul className="m-0 flex list-none flex-col gap-3 p-0">{airtableBrandSummary.map(([label, count]) => <li key={label} className="grid grid-cols-[minmax(90px,140px)_1fr_32px] items-center gap-3"><span className="truncate whitespace-nowrap text-[0.82rem] text-[var(--ink)]">{label}</span><div className="h-2.5 overflow-hidden rounded-full bg-[var(--line)]"><div className="h-full min-w-[6px] rounded-full bg-gradient-to-r from-blue-500 to-blue-300" style={{ width: `${Math.round((count / maxAirtableBrandCount) * 100)}%` }} /></div><span className="text-right text-[0.8rem] font-bold text-[var(--ink)] [font-variant-numeric:tabular-nums]">{count}</span></li>)}</ul></section>
          </div>
