...
export function DashboardJotformSection({
  ...
  return (
    <DashboardSectionPanel id="inquiries" title="JotForm">
        ...
        {jfLoading ? (
          ...
          <>
            <p className="m-0 text-[0.82rem] leading-[1.55] text-[var(--muted)]">
              This view is meant for quick intake pacing. Keep the trend and brand concentration visible, then expand the recent submission list only when you need to inspect individual leads.
            </p>
            <div className="mb-4 grid grid-cols-1 gap-[0.8rem] sm:grid-cols-3">
      ...
      <DashboardSubPanel title={<>Top Requested Brands <span className="text-[0.72rem] font-medium tracking-[0.02em] text-[var(--muted)]">From last 500 submissions</span></>} className={jfLoading ? 'min-h-[320px]' : undefined}>
        {jfLoading ? <DashboardBarListSkeleton count={6} /> : topBrands.length > 0 ? <ul className="m-0 flex list-none flex-col gap-[0.65rem] p-0">{topBrands.slice(0, 6).map(([brand, count], index) => <li key={`${brand}-${index}`} className="grid grid-cols-[130px_1fr_36px] items-center gap-3 max-[520px]:grid-cols-[90px_1fr_30px]"><span className="overflow-hidden text-ellipsis whitespace-nowrap text-[0.82rem] font-semibold text-[var(--ink)]">{brand}</span><div className="h-[7px] overflow-hidden rounded-full bg-[var(--line)]"><div className="h-full min-w-[4px] rounded-full bg-[linear-gradient(90deg,var(--accent),#5da6ff)] transition-[width] duration-[600ms] ease-in-out" style={{ width: `${toPercent(count, topBrands[0]?.[1] ?? 0)}%` }} /></div><span className="text-right text-[0.75rem] font-semibold text-[var(--muted)] [font-variant-numeric:tabular-nums]">{count}</span></li>)}</ul> : <p style={{ color: 'var(--muted)', margin: 0 }}>No brand data yet.</p>}
      </DashboardSubPanel>

      <DashboardSubPanel title={<>Recent Submissions <span className="text-[0.72rem] font-medium tracking-[0.02em] text-[var(--muted)]">Latest 8</span></>} className={jfLoading ? 'min-h-[470px]' : undefined}>
        {jfLoading ? <><DashboardTableSkeleton columns={3} rows={8} /><div className="h-9 w-52 animate-pulse rounded-lg bg-white/10" /></> : <details className="rounded-[12px] border border-[var(--line)] bg-[color:color-mix(in_srgb,var(--panel)_88%,transparent)] px-4 py-3"><summary className="cursor-pointer list-none text-[0.8rem] font-semibold text-[var(--ink)]">Show latest submissions</summary><div className="mt-3"><ul className="m-0 flex-1 list-none p-0">{jfSubmissions.slice(0, 8).map((submission, index) => {
        ...
        })}{jfSubmissions.length === 0 && <li style={{ color: 'var(--muted)', fontSize: '0.9rem' }}>No submissions loaded.</li>}</ul><button className="mt-3 cursor-pointer self-start rounded-lg border border-[var(--line)] bg-transparent px-[0.85rem] py-[0.38rem] text-[0.78rem] font-semibold text-[var(--accent)] transition-[background,border-color] duration-[140ms] hover:border-[var(--accent)] hover:bg-[var(--panel)]" onClick={() => onSelectTab('jotform')}>View all {jfSubmissions.length.toLocaleString()} submissions →</button></div></details>}
      </DashboardSubPanel>
    </DashboardSectionPanel>
  );
}
