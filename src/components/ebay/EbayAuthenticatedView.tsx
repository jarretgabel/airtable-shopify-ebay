...
export function EbayAuthenticatedView({
  ...
  return (
    <div className="flex flex-col gap-5 py-1">
      ...
      {inventoryItems.length > 0 && <><div className="mt-1 text-[0.78rem] font-extrabold uppercase tracking-[0.06em] text-[var(--muted)]">Inventory API drafts and listings</div><div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4 max-[600px]:grid-cols-1">{inventoryItems.map((item) => <EbayCard key={item.sku} item={item} offer={offerForSku(offers, item.sku)} />)}</div></>}
    </div>
  );
}
