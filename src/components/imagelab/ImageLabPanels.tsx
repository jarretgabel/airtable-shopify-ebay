...
export function ImageLabSessionStatsBar({ itemsLength, sessionStats }: { itemsLength: number; sessionStats: ImageLabSessionStats }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-0 rounded-[14px] border border-[var(--line)] bg-[var(--panel)] px-5 py-3 shadow-[0_1px_3px_rgba(17,32,49,0.06)] max-[900px]:items-stretch max-[900px]:gap-3">
      ...
      {sessionStats.brands.length > 0 && <><div className="h-[2.2rem] w-px shrink-0 bg-[var(--line)] max-[900px]:hidden" /><div className="flex flex-col items-start gap-0.5 px-5 max-[900px]:px-0"><div className="flex max-w-[260px] flex-wrap gap-1.5 max-[900px]:max-w-none">{sessionStats.brands.map((b) => <span key={b} className="whitespace-nowrap rounded-full border border-blue-400/35 bg-blue-500/20 px-[0.6em] py-[0.15em] text-[0.7rem] font-bold text-blue-200">{b}</span>)}</div><span className="whitespace-nowrap text-[0.65rem] font-semibold uppercase tracking-[0.07em] text-[var(--muted)]">Brands</span></div></>}
    </div>
  );
}
...
export function ImageLabDropZone({ dragging, itemsLength, fileInputRef, onDragOver, onDragLeave, onDrop, onFileChange }: DropZoneProps) {
  return (
    <div className={['cursor-pointer select-none rounded-[14px] border-2 border-dashed border-[var(--line)] bg-[var(--panel)] px-8 py-8 text-center text-[var(--muted)] transition-[border-color,background,color]', 'flex min-h-[180px] flex-col items-center justify-center gap-2.5', dragging ? 'border-[var(--accent)] bg-blue-500/15 text-[var(--accent)]' : '', itemsLength > 0 ? 'min-h-[52px] flex-row px-6 py-3' : ''].filter(Boolean).join(' ')} onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop} onClick={() => fileInputRef.current?.click()} role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && fileInputRef.current?.click()} aria-label="Upload images">
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-2 max-[600px]:flex-col max-[600px]:items-stretch">
      ...
      <div className="flex items-center gap-2 max-[600px]:justify-stretch [&>button]:max-[600px]:flex-1">
        ...
        <button className="rounded-lg border border-red-400/35 bg-[var(--panel)] px-4 py-2 text-[0.84rem] font-semibold text-red-300 transition hover:bg-red-500/20 disabled:cursor-not-allowed disabled:opacity-45" onClick={onClearAll}>Clear All</button>
      </div>
    </div>
  );
}
