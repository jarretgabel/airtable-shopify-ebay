...
export function canUserAccessPage(currentUser: AppUser | null, page: AppPage): boolean {
  ...
  if (page === 'settings' || page === 'notifications' || page === 'request-form') return true;
...
export function getAccessiblePages(currentUser: AppUser | null): AppPage[] {
  ...
  return Array.from(new Set([...currentUser.allowedPages.filter((page) => page !== 'users' && page !== 'settings' && page !== 'notifications' && page !== 'request-form'), 'request-form', 'settings', 'notifications']));
}
