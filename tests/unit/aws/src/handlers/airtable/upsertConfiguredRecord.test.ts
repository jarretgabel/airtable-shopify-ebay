  ...
  return {
    ...
    body: JSON.stringify({
      fields: {
        ...
        'Allowed Pages': 'dashboard,manual-intake,inventory,jotform,parking-lot,trash-review,testing-queue,photography-queue,testing,photos,listings,post-publish,archive,shopify,ebay,market,settings,notifications,imagelab',
  ...
  const handler = createUpdateHandler({
    requireRouteAccess: async (_event, override) => {
      ...
      return {
        ...
        mustChangePassword: false,
        role: 'processor',
        allowedPages: ['dashboard', 'manual-intake', 'inventory', 'jotform', 'parking-lot', 'trash-review', 'testing-queue', 'photography-queue', 'testing', 'photos', 'listings', 'post-publish', 'archive', 'shopify', 'ebay', 'market', 'settings', 'notifications', 'imagelab'],
      };
    },
